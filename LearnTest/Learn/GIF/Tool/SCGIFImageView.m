...
- (void) decodeGIF:(NSData *)GIFData
{
	...
	GIF_colorS = 2 << GIF_colorC; // 相当于 2 * 2的7次方
	
    NSLog(@"GIF_colorC==%d GIF_colorS==%d", GIF_colorC, GIF_colorS);
...
//            {
//                NSLog(@"LITTLE_ENDIAN===%d", LITTLE_ENDIAN);
//            } else if (BYTE_ORDER == BIG_ENDIAN)
//            {
//                NSLog(@"BIG_ENDIAN===%d", BIG_ENDIAN);
//            } else if (BYTE_ORDER == PDP_ENDIAN)
//            {
//                NSLog(@"PDP_ENDIAN===%d", PDP_ENDIAN);
//            }
            
			frame.delay = (buffer[1] | buffer[2] << 8); // << 优先级高于 |
            
            NSLog(@"frame.delay == %f", frame.delay);
