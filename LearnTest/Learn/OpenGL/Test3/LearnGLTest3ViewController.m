...
#import "LearnGLTest3ViewController.h"
@import OpenGLES;
#import "ShaderOperations.h"
#import "Test3TouchView.h"

@interface LearnGLTest3ViewController ()<Test3TouchViewDelegate>
{
    ...
    GLuint _glTouchProgram;    // shader程序
    GLuint _positionTouchSlot; // 用于绑定shader中的Position参数
    GLuint _colorTouchSlot;    // 用于绑定shader中的SourceColor参数
...
@implementation LearnGLTest3ViewController
...
- (void)setup
{
    ...
    [self setupBlendMode]; // 混合模式
    [self setupShaders];   // Shader脚本执行
    [self setupTexture];   // 图片转纹理
    [self setupImageToTexture];
//  [self setupImageToTexture_pbo];
    [self setupRenderTexture];// 渲染纹理像素
...
- (void)setupShaders
{
    _glProgram = [ShaderOperations compileShaderVertexName:@"Test3ShaderVertex" fragmentName:@"Test3ShaderFragment"];
    glUseProgram(_glProgram);
...
- (void)setupShaders_touch
{
    _glTouchProgram = [ShaderOperations compileShaderVertexName:@"Test3TouchVertex" fragmentName:@"Test3TouchFragment"];
    glUseProgram(_glTouchProgram);
    _positionTouchSlot = glGetAttribLocation(_glTouchProgram, "Position");
    _colorTouchSlot    = glGetUniformLocation(_glTouchProgram, "SourceColor");
}
...
- (void)setupImageToTexture_pbo  // openGLES3 支持  测试性能不佳  待查原因
{
...
//        glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);
        ...
        // 但是，当使用VBO时，glVertexAttribPointer的最后一个参数是要获取的参数在GL_ARRAY_BUFFER（每一个Vertex）的偏移量
        // 取出Vertex结构体的Position，赋给_positionSlot
        glVertexAttribPointer(_positionTouchSlot, 3, GL_FLOAT, GL_FALSE, 0, vertices);
        glEnableVertexAttribArray(_positionTouchSlot);
        
        // _colorSlot对应SourceColor参数, uniform类型, 使用glUniform4f来传递参数至shader.
        glUniform4f(_colorTouchSlot, 1.0f, 0.0f, 0.0f, 1.0f);
...
{
    [self setupRenderUsingIndexVBOWithTouchPoints:points];
}
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    [self setupRenderUsingIndexVBOWithTouchPoints:_touchPoints];
}
...
- (void)dealloc
{
    ...
    if (_glTouchProgram)
    {
        glDeleteProgram(_glTouchProgram);
        _glTouchProgram = 0;
    }
